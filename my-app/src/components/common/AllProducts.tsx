...
const AllProducts:React.FC=()=>{
    ...
    return(
        ...
        {productData.map((item,i)=>{
            return(
                <ProductContent key={i} id={item.productId} name={item.productName} brand={item.brand} image={item. productImage} price={item.productPrice} featured={item.featured} ratings={item.ratings} />
            );
           })}
           </AllProductsContainer>
        </>
